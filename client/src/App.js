...
function App() {
  ...
  useEffect(() => {
    if (user) {
      localStorage.setItem('socratic_progress', JSON.stringify(userProgress));
    }
  }, [userProgress, user]);

  useEffect(() => {
    if (user) {
      localStorage.setItem('socratic_quiz_scores', JSON.stringify(quizScores));
    }
  }, [quizScores, user]);
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('socratic_progress');
    localStorage.removeItem('socratic_quiz_scores');
    setToken(null);
    setUser(null);
    setUserProgress({});
    setQuizScores({});
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center">Loading ScortIQ...</div>;
  }
  ...
  if (!user.profileComplete) {
    return <ProfileSetup token={token} onProfileComplete={handleProfileComplete} />
  }
